...
import processorManager from './core/processorManager';
...
export function createWorkletMessenger(processorId, prefetchedSamples, addBufferToWorklet, PROCESSOR_COUNT) {
  return async function handleWorkletMessage(event) {
    ...
    // Get processor by ID using processorManager



    const processor = processorManager.getProcessorIds()[procId];

    processor.sampleRoundCnt ? processor.sampleRoundCnt++ : 1;
    ...
    if (data.type === 'nextTrigger') {
      ...
      while (changSample === procId) {
        changSample = `proc${Math.floor(Math.random() * PROCESSOR_COUNT)}`;
      }
